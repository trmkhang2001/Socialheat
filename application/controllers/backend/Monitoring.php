...
class Monitoring extends BackendController
{
	...
	private function _getMissInteraction($itemId)
	{
		$itemPerPage = ITEM_PER_PAGE_5;
		$conditions = [
			'item_id' => $itemId
		];
		$page = (int)$this->input->get('page_miss', TRUE);
		$offset = $page ? $itemPerPage * ($page - 1) : 0;
		$missInteractions = BusinessMissInteraction::getInstance()->getRangeCache($conditions, $offset, $itemPerPage);
		$total = BusinessMissInteraction::getInstance()->getCount($conditions);
		$pagination = Pagination::bootstrap($total, '', $itemPerPage, 'page_miss', 5);
		return [
